...
<div class="container">
    ...
    <div class="alerta">
        ⚠️ Clique em <strong>ATIVAR ALERTA SONORO</strong> para o sistema falar qual mesa foi liberada.
    </div>
...
function ativarSom(){
    ...
    falarTexto("Alerta sonoro ativado");
...
async function carregarMesas(){
    try{
        ...
        if(!primeiraCarga && mesasNovasLiberadas.length > 0 && somAtivo){
            tocarSom();
...
function falarMesasLiberadas(mesas){
    mesas.forEach(numero=>{
        falarTexto(`Atenção, mesa ${numero} disponível`);
    });
}
